...
def read_nfdb_point():
  ...
  with open(NFDB_POINT_FILE) as nfdb_point_csv:
    ...
    for row in nfdb_point_reader:
      ffmc = 85
      temp = (float(row['MONTH']) - 3) * 4
      hum = 0.8
      wind = 2
      x.append((row['LATITUDE'], row['LONGITUDE'], row['YEAR'], row['MONTH'], row['DAY'], ffmc, temp, hum, wind))
      
  with open(NFDB_POINT_FILE + ".combined.csv", 'w', newline='') as altitudes_csv:  
    altitudes_writer = csv.writer(altitudes_csv)
    altitudes_writer.writerow(["LATITUDE", "LONGITUDE", "YEAR", "MONTH", "DAY", "FFMC", "TEMP", "HUM", "WIND"])
    for (latitude, longitude, year, month, day, ffmc, temp, hum, wind) in x:
       altitudes_writer.writerow([latitude, longitude, year, month, day, ffmc, temp, hum, wind])
